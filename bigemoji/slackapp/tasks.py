...
def upload_bigemoji(team_id, channel_id, slack_user_id, bigemoji_name, response_url):
    ...
    try:
        ...
        token = SlackUserToken.objects.get(
            Q(scope__contains="files:write:user")
            & Q(scope__contains="chat:write:user"),
        ...
        if resp_json.get("ok"):
            ...
            for channel_info in shared_info.values():
                if channel_id in channel_info:
                    timestamp = min(
                        map(lambda info: info.get("ts", ""), channel_info[channel_id])
                    )
                    break
            ...
            slack_delayed_response(
                response_url, "Something went wrong. Please try again!"
            )
        ...
        slack_delayed_response(
            response_url,
            "You should grant us some permissions. "
...
@shared_task
def delete_bigemoji(
    team_id, channel_id, slack_user_id, bigemoji_name, timestamp, file_id, token
):
    def try_delete(trying_token):
        ...
        if resp_json.get("ok"):
            success = slack_api_call("files.delete", trying_token, file=file_id).get(
                "ok"
            )
        return success
