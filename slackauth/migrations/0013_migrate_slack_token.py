...
def reverse_copy_slack_token(apps, schema_editor):
    ...
    with transaction.atomic():
        for row in SlackToken.objects.all():
            if row.token_type == 0:
                SlackUserToken.objects.filter(account=row.account, app=row.app).delete()
            elif row.token_type == 1:
                SlackBotToken.objects.filter(
                    app=row.app, team=row.account.team
                ).delete()
...
class Migration(migrations.Migration):
    ...
    dependencies = [("slackauth", "0012_auto_20190403_1540")]

    operations = [migrations.RunPython(copy_slack_token, reverse_copy_slack_token)]
